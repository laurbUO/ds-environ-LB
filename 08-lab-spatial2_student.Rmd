...
```{r}
oba <- read.csv('/Users/laurenberger/Library/CloudStorage/Dropbox-UniversityofOregon/Lauren Berger/ds-environ/data/OBA_2018-2024.csv'
                )
...
```{r}
my_species <- oba[oba$GenusSpecies == "Halictus farinosus",]
my_species <- my_species %>%
  rename(
    x = decimalLongitude,
    y = decimalLatitude
  )
my_species_clean <- my_species %>%
  filter(!is.na(x) & !is.na(y))

my_species_sf <- st_as_sf(
  my_species_clean,
  coords = c("x", "y"),
  crs = 4326      # WGS84, the standard lat/long CRS
)
...
ggplot() +
  geom_polygon(data = or, aes(x = lon, y = lat, group = group),
               fill = "grey90", color = "black") +
  geom_sf(data = my_species_sf, color = "red", size = 2) +
  coord_sf() +
  theme_minimal() +
  labs(title = "Occurrences of Halictus farinosus in Oregon")
...
```{r plot-creative-lab}
or_ecoregion <- st_read('/Users/laurenberger/Library/CloudStorage/Dropbox-UniversityofOregon/Lauren Berger/ds-environ/labs/data/OR-ecoregions/Ecoregions_OregonConservationStrategy.shp')

ggplot() +
  geom_sf(data = or_ecoregion, aes(fill = Ecoregion), color = "grey40", alpha = 0.5) +
  geom_sf(data = my_species_sf, aes(color = month), size = 2) +
  
  annotation_scale(location = "bl", width_hint = 0.3) +
  annotation_north_arrow(location = "tl", which_north = "true", style = north_arrow_fancy_orienteering) +
  
  # Titles & theme
  labs(title = "Halictus farinosus occurrences across Oregon ecoregions",
       fill = "Ecoregion", color = "Month Collected") +
  theme_minimal() +
...
dir('/Users/laurenberger/Library/CloudStorage/Dropbox-UniversityofOregon/Lauren Berger/ds-environ-LB/Data/OR_DEM_10M.gdb.zip') #views contents of folder

gdb_path <- '/Users/laurenberger/Library/CloudStorage/Dropbox-UniversityofOregon/Lauren Berger/ds-environ-LB/Data/OR_DEM_10M.gdb.zip' #creates path for gdb folder

or.DEM <- rast(gdb_path)
summary(or.DEM)#selects raster
or.DEM.df <- as.data.frame(or.DEM, xy = TRUE)
head(or.DEM.df, n = 10)
mean.elevation <- extract(x = or.DEM,
                                  y = specDist_crop(, dist = 20),
                                  fun = mean)
ggplot() +
  geom_histogram(data = mean.elevation, aes(x = OR_DEM_10M)) + #x is the layer
  ggtitle("Histogram of Elevation Values (ft)") +
  xlab("Mean Elevation") +
  ylab("Frequency of Pixels")
or.DEM.df <- as.data.frame(or.DEM, xy = TRUE)
ggplot() +
  # DEM
  geom_raster(data = or.DEM.df, aes(fill = OR_DEM_10M)) +
  # Bee occurrences
  geom_sf(data = specDist_crop) +
  theme_minimal() +
  labs(title = "Halictus farinosus Observations in Oregon Counties & Elevation") +
  theme(plot.title = element_text(hjust = 0.5))

#unable to process map fully, this is the correct code
